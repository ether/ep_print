# Contributing to ep_print

Thanks for helping improve `ep_print` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_print` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_print from your local clone
pnpm run plugins i --path ../ep_print

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_print/` are picked up after a server restart.
...
* Etherpad version, ep_print version, browser + OS
* Steps to reproduce
